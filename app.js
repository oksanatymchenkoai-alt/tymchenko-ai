...
function addExternal(actions,label,url,secondary=false,newTab=true){const safe=safeUrl(url);if(!safe)return;const a=document.createElement('a');a.href=safe;if(newTab){a.target='_blank';a.rel='noopener noreferrer'}a.textContent=label;if(secondary)a.className='secondary';actions.append(a)}
...
function promptCard(item,index){
  ...
  copy.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(item.text);copy.textContent='Скопійовано';setTimeout(()=>copy.textContent='Копіювати',1800)}catch{copy.textContent='Виділіть текст'}});
...
function renderResource(r){
  ...
  if(prompts.length){
    ...
    const list=document.createElement('div');list.className='prompt-list';prompts.forEach((item,index)=>list.append(promptCard(item,index)));ps.append(list);
  ...
  if(pdfView||pdfDown){const pdf=section('PDF-інструкція');const p=document.createElement('p');p.textContent='PDF зберігається на Google Drive. Після перегляду поверніться на сайт кнопкою браузера «Назад».';const actions=document.createElement('div');actions.className='actions';addExternal(actions,'Відкрити PDF',pdfView,false,false);addExternal(actions,'Завантажити PDF',pdfDown,true,false);pdf.append(p,actions);root.append(pdf)}
}
...
initMenu();initCatalog();initResource();
